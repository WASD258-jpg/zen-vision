...
如果你的 DeepSeek 很强但"瞎"：看不到你贴的截图、`view_image` 全被拒、没法从图片里 debug——这个仓库就是给你的。它不换昂贵的多模态主模型，而是把图片交给 **OpenCode Zen 上的免费视觉模型**（`mimo-v2.5-free`），把**文字描述**喂给你的纯文本模型。现有 DeepSeek 配置完全不动。
...
**怎么拿 key**：在 opencode 里执行 `/connect`，选 **OpenCode Zen**，浏览器打开的页面里复制 API key。key 存在本机 `.local/share/opencode/auth.json`。
...
图片永远不会直接进 DeepSeek：由视觉模型描述，纯文本模型拿描述做推理——*Describe-then-Reason*（Prism, NeurIPS 2024）。
...
**能换别的视觉 API 吗？**
能——任何支持 `image_url` 的 OpenAI 兼容端点都行（GLM-4V、Kimi、qwen-vl、Gemini……），改 `.env` 三行即可。
